...
def quantile_lognormal_X(p, mu_V, sigma_V):
    """
    Returns the diameter value associated with a quantile
    for a lognormal distributuion
    
    Args:
        p       (float): Quantile 
        mu_V    (float): Mean of the lognormal distribution
        sigma_V (float): Standard deviation of the lognormal distribution
        
    Returns:
        float: The diameter value associated with a quantile for a lognormal distributuion
    """
...
def case_gen(simulation_name, path, trans_friction, rolling_friction, surface_energy, diameter_mean, std_deviation, n_layers):
    ...
    # The following loop helps us find how many subdivision we need to reach the appropriate cell size in z.
    # The cell size needs to be between the min/max_cell_size.
    # Every direction have the same refinement. 
    # Since the domain length in z in the smallest dimension, we start with this one. 
    ...
    def find_subdivision(total_length_dir):
        ...
        for a specific dimension length 

        Args:
            total_length_dir (float): Length Multiplier

        Returns:
            float: total_length_dir: Total length in the associated direction.
        """
