...
def case_gen(simulation_name, path, trans_friction, rolling_friction, surface_energy, n_layers, lm):
    """
    Generates a .prm file for a specific case. The .prm file is generated by replacing the symbols in a template .prm file with the right expressions.
    Also generates the .sh file for the case. The .sh file is generated by replacing the symbols in a template .sh file with the right expressions.
    Args:
        simulation_name (str): Name of the simulation. This is used to identify the simulation in question in a specific parameter file.
        path (str): Path to the folder where the .prm file will be created.
        trans_friction (float): Translational friction coefficient.
        rolling_friction (float): Rolling friction coefficient.
        surface_energy (float): Surface energy.
        n_layers (int): Number of layers in the simulation.
        lm (float): Length multiplier for the simulation.
    """
    ...
    min_dia_cutoff = diameter_values[0]
    max_dia_cutoff = diameter_values[-1]
    ...
    length_multiplier  = lm                                   # Controls the length of the domain (x direction). If set to 7, is is the real length of the experimental set-up.
