...
for i in range(len(prm_file_names)):
    ...
    d_max = float(particle.prm_dict["custom diameters"].split(",")[-1]) # Maximum particle diameter
    ...
    # This is use to find the starting time of each blades.
    ...
    location_y = np.arange(-total_height + 1. * distance_threshold, 0. -0.5 * distance_threshold,
    ...
    position = np.zeros((len(location_y) * len(location_x),2)) # Center of each square.
    rel_density = np.zeros(len(location_y )* len(location_x))  # Relative density values
    index = 0
    for px in location_x:
        ...
        for py in location_y:
            ...
            # Compute the volume in the squre
            volume_in_zone = np.sum(
                volume_cst * (df_filtered_twice["diameter"]) ** 3)
            rel_density[index] = volume_in_zone / avail_vol # Compute the relaive density
            index+=1
    ...
    rotated_density = rel_density.reshape(len(location_x), len(location_y)).transpose()
    plt.imshow(rotated_density,
               extent=[x_positions[0], x_positions[-1], y_positions[0],
                       y_positions[-1]], origin='lower',cmap="Blues") #interpolation='bilinear',
