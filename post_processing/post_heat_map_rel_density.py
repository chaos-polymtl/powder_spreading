...
avail_vol = (distance_threshold ** 2) * domain_z_length  # Volume available in the square.
...
for px in location_x:
    ...
    df_filtered = df[condition]  # We are only interested in the particle in those boundaries

    for py in location_y:
        position[index] = [px, py]
        y_min = py - 0.5 * distance_threshold  # y min of the square
        y_max = py + 0.5 * distance_threshold  # y max of the squarecd 
        condition = (y_min <= df_filtered['y']) & ( df_filtered['y'] <= y_max)
        ...
        rel_density[index] = volume_in_zone / avail_vol  # Compute the relative density
        index += 1

        x_positions = position[:, 0] - x_min_BP  # Extracting x coordinates, zeros is the beginning of the build-plate
...
cax = ax.imshow(rotated_density,
                extent=[x_positions[0], x_positions[-1], y_positions[0],
                        y_positions[-1]], origin='lower', cmap="Blues", vmin=vmin, vmax=vmax)


# Add color bar with specified limits at the top
cbar = plt.colorbar(cax, label='Relative Density', orientation="horizontal", pad=0.2)
cbar.ax.xaxis.set_ticks_position('top')
cbar.set_label('Relative Density', fontsize=14)  # Font size for the color bar label
cbar.ax.tick_params(labelsize=12)
cbar.ax.xaxis.set_ticks_position('bottom')

# Add title and labels
plt.title(r"$\mathrm{\rho_{rel}}$ on the measuring plate - 2 ", fontsize=18)  # Increase title font size
plt.xlabel("X Position", fontsize=14)  # Increase xlabel font size
plt.ylabel("Y Position", fontsize=14)


# Adjust layout to make space for colorbar at the top and xlabel at the bottom
